"""Knowledge-base search tools backed by Redis (RediSearch).

kb_search_bm25: full-text BM25 search (OR-semantics keyword query).
kb_search_vector: HNSW vector search over gemini-embedding-001 embeddings
(available only when the index was built with embeddings).

Replies are parsed via execute_command so both the classic array reply and
the Redis 8 map-style reply work regardless of redis-py version."""

import os
import re
import struct

import redis

REDIS_URL = os.environ.get("REDIS_URL", "redis://localhost:6379/0")
KB_INDEX = "kb_idx"
DOC_PREFIX = "doc:"
EMBEDDING_MODEL = "gemini-embedding-001"
EMBEDDING_DIM = 768

_client = redis.Redis.from_url(REDIS_URL, decode_responses=False)
_genai_client = None


def _get_genai_client():
    """Reused genai client (one connection pool, not a new one per search)."""
    global _genai_client
    if _genai_client is None:
        from google import genai

        _genai_client = genai.Client()
    return _genai_client


def _embed(texts: list[str]) -> list[list[float]]:
    """Embed texts with gemini-embedding-001 via google-genai."""
    from google.genai import types

    # Reduced-dim output is unnormalized; the index uses COSINE, so that's fine.
    result = _get_genai_client().models.embed_content(
        model=EMBEDDING_MODEL,
        contents=texts,
        config=types.EmbedContentConfig(output_dimensionality=EMBEDDING_DIM),
    )
    return [e.values for e in result.embeddings]


def _decode(value) -> str:
    return value.decode() if isinstance(value, bytes) else str(value)


def _parse_search_reply(reply) -> list[dict]:
    """Normalize an FT.SEARCH reply (array or map shape) to result dicts."""
    if isinstance(reply, dict):
        results = reply.get(b"results", reply.get("results")) or []
        out = []
        for row in results:
            attrs = row.get(b"extra_attributes", row.get("extra_attributes")) or {}
            doc = {"doc_id": _decode(row.get(b"id", row.get("id", "")))}
            doc.update({_decode(k): _decode(v) for k, v in attrs.items()})
            out.append(doc)
        return out
    out = []
    for i in range(1, len(reply) - 1, 2):
        doc = {"doc_id": _decode(reply[i])}
        fields = reply[i + 1]
        for j in range(0, len(fields) - 1, 2):
            doc[_decode(fields[j])] = _decode(fields[j + 1])
        out.append(doc)
    return out


def _strip_score(docs: list[dict]) -> list[dict]:
    for doc in docs:
        doc.pop("score", None)
    return docs


def kb_search_bm25(query: str, top_k: int = 5) -> list[dict]:
    """Full-text (BM25) search over the Rho-Bank knowledge base.

    Args:
        query: Keywords or a short phrase to search for. Matching is ranked,
            so extra keywords help rather than hurt.
        top_k: Number of documents to return.

    Returns:
        Matching documents with doc_id, title, and full content.
    """
    terms = re.findall(r"\w+", query.lower())
    if not terms:
        return []
    # OR-join: RediSearch defaults to AND, which zeroes out long queries.
    or_query = "|".join(dict.fromkeys(terms))
    reply = _client.execute_command(
        "FT.SEARCH", KB_INDEX, or_query,
        "LIMIT", "0", str(top_k),
        "RETURN", "2", "title", "content",
    )
    return _parse_search_reply(reply)


RRF_K = 60  # standard reciprocal-rank-fusion constant


def _rrf_fuse(*ranked_lists: list[dict], k: int = RRF_K) -> list[dict]:
    """Fuse several ranked result lists by reciprocal rank (recall-oriented).

    Uses only rank position, so BM25 scores and cosine distances (different
    scales) never need normalizing. Dedups by doc_id; first occurrence wins
    for the stored doc body.
    """
    scores: dict[str, float] = {}
    docs_by_id: dict[str, dict] = {}
    for ranked in ranked_lists:
        for rank, doc in enumerate(ranked):
            doc_id = doc.get("doc_id")
            if not doc_id:
                continue
            scores[doc_id] = scores.get(doc_id, 0.0) + 1.0 / (k + rank + 1)
            docs_by_id.setdefault(doc_id, doc)
    return sorted(docs_by_id.values(), key=lambda d: scores[d["doc_id"]], reverse=True)


def _cosine(a: list[float], b: list[float]) -> float:
    """Cosine similarity (reduced-dim gemini embeddings are unnormalized)."""
    dot = sum(x * y for x, y in zip(a, b))
    na = sum(x * x for x in a) ** 0.5
    nb = sum(y * y for y in b) ** 0.5
    return dot / (na * nb) if na and nb else 0.0


def _fetch_doc_vectors(doc_ids: list[str]) -> dict[str, list[float]]:
    """Pull stored float32 embeddings for doc_ids from Redis in one pipeline."""
    pipe = _client.pipeline(transaction=False)
    for doc_id in doc_ids:
        pipe.hget(doc_id, "embedding")
    vectors: dict[str, list[float]] = {}
    for doc_id, raw in zip(doc_ids, pipe.execute()):
        if raw:
            vectors[doc_id] = list(struct.unpack(f"{EMBEDDING_DIM}f", raw))
    return vectors


def kb_search(query: str, top_k: int = 3, candidate_k: int = 20) -> list[dict]:
    """Search the Rho-Bank knowledge base (hybrid BM25 + semantic, reranked).

    This is the PRIMARY search tool — prefer it over kb_search_bm25 /
    kb_search_vector. It runs both keyword and semantic search, fuses the
    rankings (RRF) so a relevant doc found by either retriever surfaces, then
    reranks the fused pool by semantic similarity and returns the best matches.

    Args:
        query: Keywords or a natural-language question describing what you need
            (a procedure, eligibility rule, tool name, fee table, etc.).
        top_k: Number of documents to return (default 5).
        candidate_k: Size of the fused candidate pool before reranking.

    Returns:
        The most relevant documents, each with doc_id, title, and full content.
    """
    bm25_docs = kb_search_bm25(query, top_k=candidate_k)

    query_vec: list[float] | None = None
    vector_docs: list[dict] = []
    try:
        query_vec = _embed([query])[0]
        packed = struct.pack(f"{EMBEDDING_DIM}f", *query_vec)
        reply = _client.execute_command(
            "FT.SEARCH", KB_INDEX, f"*=>[KNN {candidate_k} @embedding $vec AS score]",
            "PARAMS", "2", "vec", packed,
            "SORTBY", "score",
            "LIMIT", "0", str(candidate_k),
            "RETURN", "3", "title", "content", "score",
            "DIALECT", "2",
        )
        vector_docs = _strip_score(_parse_search_reply(reply))
    except Exception:
        # No embeddings/creds: degrade to BM25-only (still useful).
        if not bm25_docs:
            return []

    fused = _rrf_fuse(bm25_docs, vector_docs)[:candidate_k]

    # Semantic rerank of the fused pool when we have a query vector.
    if query_vec and fused:
        doc_vectors = _fetch_doc_vectors([d["doc_id"] for d in fused])
        fused.sort(
            key=lambda d: _cosine(query_vec, doc_vectors.get(d["doc_id"], [])),
            reverse=True,
        )

    return fused[:top_k]


def kb_search_vector(query: str, top_k: int = 5) -> list[dict]:
    """Semantic (vector) search over the Rho-Bank knowledge base.

    Better than kb_search_bm25 when the query is a natural-language question
    rather than exact keywords.

    Args:
        query: A natural-language question or description.
        top_k: Number of documents to return.

    Returns:
        Matching documents with doc_id, title, and full content; or an error
        entry telling you to fall back to kb_search_bm25.
    """
    try:
        vector = struct.pack(f"{EMBEDDING_DIM}f", *_embed([query])[0])
        reply = _client.execute_command(
            "FT.SEARCH", KB_INDEX, f"*=>[KNN {top_k} @embedding $vec AS score]",
            "PARAMS", "2", "vec", vector,
            "SORTBY", "score",
            "LIMIT", "0", str(top_k),
            "RETURN", "3", "title", "content", "score",
            "DIALECT", "2",
        )
        return _strip_score(_parse_search_reply(reply))
    except Exception as e:
        return [
            {
                "error": f"Vector search unavailable ({type(e).__name__}). "
                "Use kb_search_bm25 with keywords instead."
            }
        ]
